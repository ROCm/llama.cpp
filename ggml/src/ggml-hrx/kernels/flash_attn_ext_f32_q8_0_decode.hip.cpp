...
struct hrx_flash_attn_ext_f32_q8_0_decode_constants {
    long long D;
    long long KV;
    long long N;
    long long H;
...
static __device__ __forceinline__ float hrx_load_q8_0(const hrx_block_q8_0 * base, long long d) {
    const hrx_block_q8_0 * block = base + d / 32;
    return __half2float(__ushort_as_half(block->d)) * static_cast<float>(block->qs[d & 31]);
}
...
static __device__ __forceinline__ float hrx_alibi_slope(
        const hrx_flash_attn_ext_f32_q8_0_decode_constants c,
        long long head) {
    if (c.max_bias <= 0.0f) {
        return 1.0f;
    }
...
extern "C" __global__ void hrx_flash_attn_ext_f32_q8_0_decode(
        const float * q, const hrx_block_q8_0 * k, const hrx_block_q8_0 * v,
        const __half * mask, const float * sinks, float * dst,
        hrx_flash_attn_ext_f32_q8_0_decode_constants c) {
    ...
    for (long long t = tid; t < c.KV; t += 256) {
        ...
        for (long long d = 0; d < c.D; ++d) {
            ...
            score += qv * hrx_load_q8_0(reinterpret_cast<const hrx_block_q8_0 *>(k_row), d);
        }
        score *= c.scale;
    ...
    for (long long d = 0; d < c.D; ++d) {
        ...
        for (long long t = tid; t < c.KV; t += 256) {
            ...
            local += logits[t] * inv_sum * hrx_load_q8_0(reinterpret_cast<const hrx_block_q8_0 *>(v_row), d);
        }
