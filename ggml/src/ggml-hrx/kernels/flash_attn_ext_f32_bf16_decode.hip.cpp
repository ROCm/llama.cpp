...
struct hrx_flash_attn_ext_f32_bf16_decode_constants {
    long long D;
    long long KV;
    long long N;
    long long H;
...
static __device__ __forceinline__ float hrx_load_bf16(const uint16_t * base, long long byte_offset) {
    return hrx_bf16_to_f32(*reinterpret_cast<const uint16_t *>(reinterpret_cast<const char *>(base) + byte_offset));
}
...
static __device__ __forceinline__ float hrx_alibi_slope(
        const hrx_flash_attn_ext_f32_bf16_decode_constants c,
        long long head) {
    if (c.max_bias <= 0.0f) {
        return 1.0f;
    }
...
extern "C" __global__ void hrx_flash_attn_ext_f32_bf16_decode(
        const float * q, const uint16_t * k, const uint16_t * v, const __half * mask, const float * sinks, float * dst,
        hrx_flash_attn_ext_f32_bf16_decode_constants c) {
    ...
    for (long long t = tid; t < c.KV; t += 256) {
        ...
        for (long long d = 0; d < c.D; ++d) {
            ...
            score += qv * hrx_load_bf16(reinterpret_cast<const uint16_t *>(k_row), d * static_cast<long long>(sizeof(uint16_t)));
        }
        score *= c.scale;
    ...
    for (long long d = 0; d < c.D; ++d) {
        ...
        for (long long t = tid; t < c.KV; t += 256) {
            ...
            local += logits[t] * inv_sum *
                hrx_load_bf16(reinterpret_cast<const uint16_t *>(v_row), d * static_cast<long long>(sizeof(uint16_t)));
        }
