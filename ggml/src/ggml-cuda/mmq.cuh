...
#pragma unroll
                for (int l = 0; l < tile_C::ne; ++l) {
                    ...
                    sum[(j0/tile_C::J + n)*tile_C::ne + l] +=
                        C.x[l] * sc[k01/4] * x_df[i*sram_stride] * dB0;
                }
            }
...
// ---------------------------------------------------------------------------------------------
        ...
        case GGML_TYPE_Q6_K:
            return ggml_cuda_mmq_util_funcs(
                -1,
                ggml_cuda_mmq_load_tiles_q6_K<type, J, fallback>,
                ggml_cuda_mmq_vec_dot_q6_K_q8_1_mma_rdna35<type, J, fallback>,
                ggml_cuda_mmq_write_back_mma<type, J, fallback>);
// ---------------------------------------------------------------------------------------------
        case GGML_TYPE_IQ1_S:
            return ggml_cuda_mmq_util_funcs(
...
#if defined(RDNA3_5)
template <int J, int nwarps, int warp_size, int nchunks>
static __device__ __forceinline__ void mmq_tile_y_load_global(
        int * __restrict__ tile_y, const int * __restrict__ src) {
#pragma unroll
...
static __device__ __forceinline__ void mmq_tile_y_load_global_to_regs(
        const int * __restrict__ src, int (&cache)[nchunks]) {
#pragma unroll
    for (int c = 0; c < nchunks; ++c) {
        const int l = c*(nwarps*warp_size) + threadIdx.y*warp_size + threadIdx.x;
        cache[c] = src[l];
...
#if defined(RDNA3_5)
    int * tile_x = tile_y + GGML_PAD(
        J*MMQ_TILE_Y_K, (type == GGML_TYPE_Q4_K ? 1 : 2)*nwarps*warp_size);
#else
    ...
    if constexpr (J <= 64 && tile_y_elems % tile_y_load_stride == 0) {
        ...
        for (int kb0 = kb0_start; kb0 < kb0_stop; kb0 += blocks_per_iter) {
            ...
            if (have_y0_prefetch) {
                ...
                mmq_tile_y_load_global<J, nwarps, warp_size, tile_y_nchunks>(tile_y, by0);
            }
            __syncthreads();
...
static size_t mmq_get_nbytes_shared(const ggml_cuda_mmq_config & config, const int cc) {
    ...
    const size_t alignment = (GGML_CUDA_CC_IS_RDNA3_5(cc) ? 2 : 1) * config.nthreads*sizeof(int);
    return nbs_ids + nbs_x + GGML_PAD(nbs_y, alignment);
}
...
static void launch_mul_mat_q(ggml_backend_cuda_context & ctx, const mmq_args & args, cudaStream_t stream) {
    ...
    if (!ggml_cuda_mmq_get_stream_k(type, J, fallback, cc)) {
        ...
        if (use_compact) {
            ...
            const int64_t max_m_blocks =
                (args.ncols_dst + int64_t(n_experts)*(J - 1) + J - 1) / J;
            if (max_m_blocks < MMQ_MAX_GRIDDIM_Y) {
                block_start.alloc(n_experts + 1);
                block_expert.alloc(max_m_blocks);
                constexpr int build_nthreads = 256;
...
void mul_mat_q_switch_J(ggml_backend_cuda_context & ctx, const mmq_args & args, cudaStream_t stream) {
    ...
    if (args.expert_bounds != nullptr && GGML_CUDA_CC_IS_RDNA3_5(cc)) {
        ...
        ncols_to_tile = std::min(args.ncols_max, 2*ncols_per_expert);
    }
    ...
    for (int J = 8; J <= 128 && ntiles_J_best > 1; J += 8) {
        ...
        const int ntiles_x = (ncols_to_tile + config.J - 1) / config.J;
    ...
    if (GGML_CUDA_CC_IS_RDNA3_5(cc) && J_best > 0 && dual_wg_type) {
        ...
        if (mmq_get_nbytes_shared(ggml_cuda_mmq_get_config(type, J_best, fallback, cc), cc) > lds_dual_wg) {
            int J_dual = 0;
            size_t nbytes_min = SIZE_MAX;
            for (int J = 8; J <= 128; J += 8) {
                const ggml_cuda_mmq_config config = ggml_cuda_mmq_get_config(type, J, fallback, cc);
                if (config.type == GGML_TYPE_COUNT) {
                    continue;
                }
                const size_t nbytes = mmq_get_nbytes_shared(config, cc);
                const int ntiles = (args.ncols_max + J - 1) / J;
                if (nbytes <= lds_dual_wg && ntiles <= 2*ntiles_J_best && nbytes < nbytes_min) {
                    J_dual = J;
                    nbytes_min = nbytes;
                }
            }
    ...
    if (GGML_CUDA_CC_IS_RDNA3_5(cc) && J_best > 0) {
        const int J_tuned = mmq_rdna35_tuned_J(type, args.expert_bounds != nullptr, J_best);
...
void mul_mat_q_case(ggml_backend_cuda_context & ctx, const mmq_args & args, cudaStream_t stream) {
    ...
    const int I = GGML_CUDA_CC_IS_RDNA3_5(cc) ? 64 : 128;
    if (args.nrows_x % I == 0) {
