...
// ============================================================================
// ROCmFP4 (AMD gfx1151): Q4_0-layout FP4 with per-half UE4M3 scales.
// The codebook is E2M1-derived at half-scale, with the top magnitude retuned to 12 ->10.
// Decoded values live in kvalues_rocmfp4 (== 2*E2M1 with that retune), and UE4M3 scale bytes decode via ggml_ue4m3_to_fp32 (raw*0.5, "half" scale).
// The scale-search/quantize path below keeps a local half-scale table and an arithmetic decoder so the exhaustive MSE search avoids per-candidate rebuilds.
// ============================================================================
...
// every candidate during exhaustive scale search. Values come from the shared
// GGML_ROCMFP4_SCALE_UE4M3_HALF_LIST in ggml-common.h (single source of truth).
static const float rocmfp4_scale_ue4m3_half[127] = { GGML_ROCMFP4_SCALE_UE4M3_HALF_LIST };
...
static inline uint8_t rocmfp4_best_index_scaled_finite(float x, float inv_scale_half) {
    ...
    //   0, +/-1, +/-2, +/-3, +/-4, +/-6, +/-8, +/-10
    // Ties intentionally choose the lower-magnitude code, matching the former linear scan because the positive codes and zero appear first.
...
// Dequant/vec_dot decode via the shared codebook (kvalues_rocmfp4) and shared
// UE4M3 decoder (ggml_ue4m3_to_fp32, raw*0.5) — bit-identical to the former
// local rocmfp4_decode_table / rocmfp4_ue4m3_to_fp32_half.
void rocmfp4_dequantize_row_q4_0(const block_rocmfp4 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_ROCMFP4 == 0);

    const int64_t nb = k / QK_ROCMFP4;
    for (int64_t ib = 0; ib < nb; ++ib) {
        const float d0 = ggml_ue4m3_to_fp32(x[ib].e[0]);
        const float d1 = ggml_ue4m3_to_fp32(x[ib].e[1]);

        for (int j = 0; j < QK_ROCMFP4/2; ++j) {
            y[ib*QK_ROCMFP4 + j]                 = (float) kvalues_rocmfp4[x[ib].qs[j] & 0x0f] * d0;
            y[ib*QK_ROCMFP4 + j + QK_ROCMFP4/2] = (float) kvalues_rocmfp4[x[ib].qs[j] >> 4]   * d1;
        }
    }
}
...
void rocmfp4_dequantize_row_q4_0_fast(const block_rocmfp4_fast * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    ...
    const int64_t nb = k / QK_ROCMFP4;
    for (int64_t ib = 0; ib < nb; ++ib) {
        const float d = ggml_ue4m3_to_fp32(x[ib].e);

        for (int j = 0; j < QK_ROCMFP4/2; ++j) {
            y[ib*QK_ROCMFP4 + j]                 = (float) kvalues_rocmfp4[x[ib].qs[j] & 0x0f] * d;
            y[ib*QK_ROCMFP4 + j + QK_ROCMFP4/2] = (float) kvalues_rocmfp4[x[ib].qs[j] >> 4]   * d;
        }
    }
}
